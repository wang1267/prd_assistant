...
- **AI 生成与问答边界**：AI 先以自然语言动态澄清用户、现状、目标、MVP 范围和不可接受结果；每累计 10 个用户回答会询问继续完善或查看可编辑方案并生成，绝不强制结束。Vibe Coding 仅表示开发方式，默认不将 AI 写成产品能力；仅在它会影响方案时追问产品是否需要 AI 功能。AI 推导统一标为待确认假设，不强迫新手填写专业指标；项目名称必须由用户在生成前填写，AI 名称只可作为参考。从想法开始始终默认使用独立的通用产品 PRD 目录，不继承当前项目或旧 AI 专属模板；用户可在确认页改选精简 MVP。
- **项目助手**：对话助手每轮带入当前项目正文、框架、项目上下文、质量缺口、待确认修改与近期评审；它可先以小白语言解释概念、引用章节和提出下一步，只有明确要求变更时才调用看板动作。
- **模型接入**：不预设收费服务或模型。用户可填写任意 OpenAI 兼容服务；标准模型用于澄清/生成，快速模型用于聊天，深度模型用于体检/优化/评审。选中服务商后自动填入官方 Base URL 与推荐模型，在线免费额度不作长期承诺。v19.17 起不再提供本地部署（Ollama）引导。
