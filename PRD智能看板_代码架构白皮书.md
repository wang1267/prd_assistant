...
>   ⚠️ v17.20 变更提示（自定义模板存取 + 总览排序）：
>   1. **自定义模板**：新函数 `tplCustomList/tplRenderPresets/tplSaveAsCustom/tplDeleteCustom`，localStorage 键 `prdKanbanTplCustom`（`[{id,name,text,at}]`）；`tplApplyPreset` 支持 `custom:<id>`；`showTpl` 打开时调用 `tplRenderPresets()`；工具条新增 `data-act="tpl-saveas" / "tpl-delcustom"`（bindStatic 新 case）；测试钩子 `window.__tplSaveName/__tplDelOk`。
