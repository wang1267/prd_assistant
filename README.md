...
- **真实联网竞品研究**：AI 先确认本轮信息，再单独询问是否搜索；搜索中清晰显示进度并可停止，真实来源与结论会写入本次需求记忆，再据此追问，未取得来源不生成模拟结论。
...
联网竞品研究优先顺序为 DeepSeek、Qwen / 阿里云百炼、GLM / 智谱。选择服务商后填入自己的地址、API Key 与深度模型：DeepSeek 使用官方 `https://api.deepseek.com` 与支持 `web_search` 的模型；Qwen 使用百炼控制台提供的 OpenAI 兼容地址与支持联网的模型；GLM 使用正常聊天配置，搜索由应用调用其官方 Web Search API。搜索时可随时停止；只有带可展示来源的结果才会写入本轮需求记忆，并由 AI 根据它提出下一件最值得确认的事。任一服务商没有返回可展示来源时，应用会明确停止，不会用普通回答伪装检索。
...
当前产品基线为 **v19.11**（2026-09-02）。版本约定：v19 之后依次使用 v19.01–v19.19；v19.19 的下一次改动进入 v20。修改主文件后，请先同步测试副本并运行：
