...
function aiGenSectionPrompt(sid,desc,fwList){
...
function aiGenSection(sid,desc,opts){
  ...
  var p=aiGenSectionPrompt(sid,desc,fwList);
...
function aiGenStart(){
  ...
  aiCancelFlag=false;
  aiStatus='';aiStatusLog=[];
  var dbg={at:Date.now(),name:name,fwId:(fwSel&&fwSel.value)||'default',sections:0,ok:0,failed:[],steps:[]};
  ...
  function next(){
    ...
    return aiGenSection(s.id,text,{onStatus:aiSetStatus}).then(function(ch){
