...
function aiCreateVersion(kind,label,patches,scoreBefore,scoreAfter,opts){
  ...
  var vs=st.versions||(st.versions=[]);
  var v={id:aiUid(),kind:kind,label:label,createdAt:Date.now(),parentId:vs.length?vs[vs.length-1].id:null,patch:patches||{},scoreBefore:scoreBefore==null?null:aiRound(scoreBefore),scoreAfter:scoreAfter==null?null:aiRound(scoreAfter),applied:!!(opts&&opts.applied),rolledBack:false};
  vs.push(v);
...
function aiReversePatch(sid,entries){
  ...
  var f=aiReverseEntries({html:c.html,rows:c.rows?aiDeep(c.rows):null},entries);
...
function aiRestoreToVersion(vid){
  ...
  var isTransform=Object.keys(target.patch||{}).some(function(sid){
  ...
  if(!later.length&&isTransform){
    ...
    try{render();refreshHealthUI();}catch(e){}
    aiRenderPanel();
    aiToast('已撤销「'+target.label+'」（内容已搬回原位）');
    return;
  }
  ...
  Object.keys(target.patch||{}).forEach(function(sid){
    ...
    if(Array.isArray(tp)){
      var c=DATA[sid];if(!c)return;
      var f=aiEnsureTarget({html:c.html,rows:c.rows?aiDeep(c.rows):null},tp);
      if(f.html!=null)c.html=f.html;
      if(f.rows)c.rows=f.rows;
...
function aiFinalizePending(){
  ...
  var kind=(pd.scoreAfter>=pd.target)?'final':'ai';
  var vs=st.versions||(st.versions=[]);
  var label=aiVersionLabel(kind,kind==='ai'?vs.filter(function(v){return v.kind==='ai';}).length+1:1);
  aiCreateVersion(kind,label,patches,pd.scoreBefore,pd.scoreAfter,{applied:true});
...
function aiRenderPanel(){
  ...
  if(!proj){
    ...
    body.innerHTML='<div class="empty">请先创建或打开一个项目，再使用 AI 助手。</div>'+recHtml;
    return;
  }
  var st=aiState();
  var eng=HEALTH&&HEALTH.metrics?'规则引擎：'+HEALTH.metrics.completion+'%':'';
  var hint=aiAlignHint();
  var html='<div class="ai-tools"><button class="ai-btn primary" data-ai="score">AI 深度体检</button><button class="ai-btn" data-ai="optimize">一键优化</button><button class="ai-btn" data-ai="align">结构对齐</button><span class="ai-engine muted">'+eng+'</span></div>';
  ...
  if(st.pendingDiffs&&st.pendingDiffs.items&&st.pendingDiffs.items.length){
    ...
    html+='<div class="ai-sec warn"><div class="ai-sec-h">待确认修改（'+pd.items.length+' 条）<span class="muted">'+pd.scoreBefore+' → '+pd.scoreAfter+' 分 · 目标 '+pd.target+'</span></div>';
...
function aiBind(){
  ...
  document.addEventListener('keydown',function(e){
    if(e.key==='Escape'){var mm=document.getElementById('aiOptModal');if(mm&&mm.classList.contains('open')){try{closeModal('aiOptModal');}catch(e2){mm.classList.remove('open');}}else if(aiUi.open)aiClosePanel();}
  });
}
function aiBoot(){
  aiInjectStyle();
